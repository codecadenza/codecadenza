...
public class MinFloatValueValidator implements ConstraintValidator {
	...
	public ConstraintViolation validate(Object objValue, Annotation constraint) {
		...
		final var c = (MinFloatValue) constraint;

		if (objValue == null)
			return null;

		if ((objValue instanceof final Float floatValue && floatValue < c.value())
				|| (objValue instanceof final Double doubleValue && doubleValue < c.value())) {
			if (c.message() != null && !c.message().isEmpty())
				result = new ConstraintViolation(c.message());
			else
				result = new ConstraintViolation("Min. float value constraint violated!");
		}
