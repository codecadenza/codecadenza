 ...
 * @version 1.0.0
 */
public class MaxDecimalValueValidator implements ConstraintValidator {
	/*
	 * (non-Javadoc)
	...
	public ConstraintViolation validate(Object objValue, Annotation constraint) {
		...
		final var c = (MaxDecimalValue) constraint;

		if (objValue == null)
			return null;

		if (objValue instanceof final BigDecimal value) {
			final var maxValue = new BigDecimal(c.value());

			if (value.compareTo(maxValue) > 0)
				if (c.message() != null && !c.message().isEmpty())
					result = new ConstraintViolation(c.message());
				else
					result = new ConstraintViolation("Max. decimal value constraint violated!");
		}
