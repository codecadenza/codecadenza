...
public class MinDecimalValueValidator implements ConstraintValidator {
	...
	public ConstraintViolation validate(Object objValue, Annotation constraint) {
		...
		final var c = (MinDecimalValue) constraint;

		if (objValue == null)
			return null;

		if (objValue instanceof final BigDecimal value) {
			final var minValue = new BigDecimal(c.value());

			if (value.compareTo(minValue) < 0)
				if (c.message() != null && !c.message().isEmpty())
					result = new ConstraintViolation(c.message());
				else
					result = new ConstraintViolation("Min. decimal value constraint violated!");
		}
