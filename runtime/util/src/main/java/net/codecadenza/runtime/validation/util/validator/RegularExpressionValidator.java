...
public class RegularExpressionValidator implements ConstraintValidator {
	...
	public ConstraintViolation validate(Object objValue, Annotation constraint) {
		...
		final var c = (RegularExpression) constraint;
		final String value = (objValue != null ? objValue.toString() : null);

		if (value != null && !matches(value, c.value())) {
			if (c.message() != null && !c.message().isEmpty())
				result = new ConstraintViolation(c.message());
			else
				result = new ConstraintViolation("Field content does not conform to the pattern!");
		}
	 ...
	 * @param value
	 * @param pattern
	 * @return true if the value matches the given pattern
	 */
