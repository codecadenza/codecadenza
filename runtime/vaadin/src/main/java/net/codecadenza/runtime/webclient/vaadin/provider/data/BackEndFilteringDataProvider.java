...
/**
 * <p>
 * A data provider that loads items from a back-end depending on the provided filter. In contrast to a
 * {@link AbstractBackEndDataProvider} this data provider only needs one callback function in order to provide the items! This is
 * necessary as CodeCadenza doesn't create corresponding count methods for simple look-up operations.
 * </p>
 * <p>
...
public class BackEndFilteringDataProvider<T> extends AbstractBackEndDataProvider<T, String> {
	 ...
	 */
	public static <T> BackEndFilteringDataProvider<T> fromCallback(FetchWithFilterCallback<T> fetchCallback) {
		return new BackEndFilteringDataProvider<>(fetchCallback);
	}
	 ...
	 * @param fetchCallback
	 */
	public BackEndFilteringDataProvider(FetchWithFilterCallback<T> fetchCallback) {
	...
	protected Stream<T> fetchFromBackEnd(Query<T, String> query) {
		// Return an empty stream if no filter has been entered!
		if (!query.getFilter().isPresent() || items == null)
	...
	protected int sizeInBackEnd(Query<T, String> query) {
		// Do not perform a count operation if no filter has been entered!
		...
		try {
			...
			items = fetchCallback.fetchData(query.getFilter().get());
