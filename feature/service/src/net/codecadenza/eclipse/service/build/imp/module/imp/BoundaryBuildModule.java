...
/**
 * <p>
 * Build module for boundary artifacts
 * </p>
 * <p>
 ...
 * @version 1.0.0
 */
public class BoundaryBuildModule extends AbstractBuildModule {
	/**
	 * Constructor
	 * @param buildArtifact
	 */
	public BoundaryBuildModule(BuildArtifact buildArtifact) {
		super(buildArtifact);
	}
	...
	public List<WorkspaceFile> getConfigurationFiles() {
		...
		addBeansXML(configFiles, BuildArtifactType.BOUNDARY);
	...
	public List<String> getPackages() {
		...
		project.getBoundaryNamespace().getChildNamespaces().forEach(ns -> {
			packages.add(project.getBoundaryNamespace().toString() + "." + ns.getName());

			if (project.isAddBoundaryInterface())
				packages.add(project.getBoundaryNamespace().toString() + "." + ns.getName() + SUB_PACKAGE_BEAN);
		});
