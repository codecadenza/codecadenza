...
public class KafkaIntegrationProjectFilesGenerator extends AbstractIntegrationProjectFilesGenerator {
	...
	private String createBaseClient() {
		...
		b.append("private static final String PROP_NAME_BROKER_URL = \"broker.url\";\n");
		b.append("private static final String PROP_NAME_GROUP_ID = \"group.id\";\n\n");
		...
		b.append("private static final String PROP_MAX_NUMBER_OF_POLLS = \"response.max.polls\";\n\n");
		...
		b.append("protected final int maximumNumberOfPolls;\n\n");
		b.append("/**\n");
		...
		b.append("this.maximumNumberOfPolls = propertyService.getIntProperty(PROP_MAX_NUMBER_OF_POLLS);\n");
		...
		b.append("try\n");
		b.append("{\n");
		b.append("Long lastOffset = null;\n\n");
		b.append("if(partitionId == null)\n");
		b.append("{\n");
		b.append("consumer.assign(Collections.emptyList());\n");
		b.append("consumer.subscribe(List.of(topic));\n");
		b.append("}\n");
		b.append("else\n");
		b.append("{\n");
		b.append("final var topicPartition = new TopicPartition(topic, partitionId);\n");
		b.append("final var partitionList = List.of(topicPartition);\n\n");
		b.append("consumer.subscribe(Collections.emptyList());\n");
		b.append("consumer.assign(partitionList);\n\n");
		b.append("// Determine the last offset\n");
		b.append("lastOffset = consumer.endOffsets(partitionList, offsetTimeout).get(topicPartition);\n\n");
		b.append("if(lastOffset != null)\n");
		b.append("{\n");
		b.append("logger.debug(\"The last offset is {}\", lastOffset);\n\n");
		b.append("// In the case of concurrent operations on the same partition it might happen that another\n");
		b.append("// consumer has already committed an offset that is greater than the offset of this operation.\n");
		b.append("// Thus, it is necessary to poll more messages in order to have a chance to find the correct one!\n");
		b.append("if(lastOffset > lookBackSize)\n");
		b.append("consumer.seek(topicPartition, lastOffset - lookBackSize);\n");
		b.append("else\n");
		b.append("consumer.seek(topicPartition, 0);\n");
		b.append("}\n");
		b.append("}\n\n");
		b.append("// A single poll might not be enough to find the proper response message!\n");
		b.append("for(int i = 0; i < maximumNumberOfPolls; i++)\n");
		b.append("for(ConsumerRecord<String, byte[]> consumerRecord : consumer.poll(responseTimeout))\n");
		b.append("{\n");
		b.append("logger.debug(\"Received response message from partition {} with offset {}\", ");
		b.append("consumerRecord.partition(), consumerRecord.offset());\n\n");
		b.append("if(!checkCorrelationId(consumerRecord, correlationId))\n");
		b.append("continue;\n\n");
		b.append("final var deserializer = new AvroObjectDeserializer<T>(responseObj.getSchema());\n");
		b.append("final var response = deserializer.deserialize(consumerRecord.value());\n\n");
		b.append("if(lastOffset == null || consumerRecord.offset() > lastOffset)\n");
		b.append("consumer.commitSync();\n\n");
		b.append("consumer.unsubscribe();\n\n");
		b.append("return response;\n");
		b.append("}\n");
		b.append("}\n");
		...
		b.append("}\n\n");
		b.append("throw new RemoteOperationException(\"Response message with correlation ID '\"");
		b.append(" + correlationId + \"' was not found!\");\n");
		b.append("}\n\n");
		b.append("/**\n");
		...
		b.append(" */\n");
		b.append("protected boolean checkCorrelationId(ConsumerRecord<String, byte[]> consumerRecord, UUID correlationId)\n");
		b.append("{\n");
		...
		b.append("props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, propertyService.getStringProperty(PROP_NAME_BROKER_URL));\n");
		b.append("props.put(ConsumerConfig.GROUP_ID_CONFIG, propertyService.getStringProperty(PROP_NAME_GROUP_ID));\n");
		...
		b.append("{\n");
		b.append("final var props = new Properties();\n");
		b.append("props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, propertyService.getStringProperty(PROP_NAME_BROKER_URL));\n");
	...
	private String createClientConfig() {
		...
		b.append("response.timeout=1000\n");
		b.append("response.max.polls=10\n");
		b.append("response.offset.timeout=500\n");
