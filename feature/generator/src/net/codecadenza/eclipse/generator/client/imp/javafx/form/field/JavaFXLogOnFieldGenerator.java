 ...
 * @version 1.0.0
 */
public class JavaFXLogOnFieldGenerator extends AbstractJavaFXFieldGenerator {
	 ...
	 * @param formGenerator
	 */
	public JavaFXLogOnFieldGenerator(FormField field, AbstractJavaSourceGenerator formGenerator) {
	...
	public String getSaveDataFragment(String objectName) {
		...
		if (project.isBoundaryMode()) {
			...
			b.append(field.getDTOAttribute().getReferencedDTOBean().getModelClassName() + ".class, ");
			b.append(SECURITY_MANAGER + ".getLogOnDTO()." + logOnDTO.getPKAttribute().getGetterName() + "));\n");
		}
