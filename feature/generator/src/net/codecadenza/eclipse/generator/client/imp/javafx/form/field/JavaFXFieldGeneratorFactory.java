 ...
 * @version 1.0.0
 */
public class JavaFXFieldGeneratorFactory {
	/**
	 * Prevent instantiation
	 */
	private JavaFXFieldGeneratorFactory() {
	...
	public static IFieldGenerator getFieldGenerator(AbstractJavaSourceGenerator formGenerator, FormField field,
		...
		if (type == FormFieldTypeEnumeration.CHECKBOX)
			fieldGenerator = new JavaFXCheckBoxFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.COMBOBOX)
			fieldGenerator = new JavaFXComboboxFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.DATE)
			fieldGenerator = new JavaFXDateFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.DATE_TIME)
			fieldGenerator = new JavaFXDateTimeFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.ENUM_COMBOBOX)
			fieldGenerator = new JavaFXEnumComboFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.LIST)
			fieldGenerator = new JavaFXListFieldGenerator(field, formGenerator, false);
		else if (type == FormFieldTypeEnumeration.LOV)
			fieldGenerator = new JavaFXListOfValuesFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.SIMPLE_TEXT || type == FormFieldTypeEnumeration.MULTI_LINE_TEXT)
			fieldGenerator = new JavaFXTextFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.PROPOSAL_TEXT)
			fieldGenerator = new JavaFXProposalTextFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.SEARCHABLE_LIST)
			fieldGenerator = new JavaFXListFieldGenerator(field, formGenerator, true);
		else if (type == FormFieldTypeEnumeration.SELECTION_BY_PARENT_FORM)
			fieldGenerator = new JavaFXSelectionByParentFormFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.SELECTION_BY_SECURITY_DTO)
			fieldGenerator = new JavaFXLogOnFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.DOCUMENT_SIZE_FIELD)
			fieldGenerator = new JavaFXDocumentSizeFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.LABEL || type == FormFieldTypeEnumeration.MULTI_LINE_LABEL)
			fieldGenerator = new JavaFXLabelFieldGenerator(field, formGenerator);
		else if (type == FormFieldTypeEnumeration.WEB_LINK || type == FormFieldTypeEnumeration.MAIL_LINK)
			fieldGenerator = new JavaFXExtLinkFieldGenerator(field, formGenerator);
