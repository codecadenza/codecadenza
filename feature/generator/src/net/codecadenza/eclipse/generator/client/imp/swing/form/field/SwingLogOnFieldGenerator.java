 ...
 * @version 1.0.0
 */
public class SwingLogOnFieldGenerator extends AbstractSwingFieldGenerator {
	 ...
	 * @param formGenerator
	 */
	public SwingLogOnFieldGenerator(FormField field, AbstractJavaSourceGenerator formGenerator) {
	...
	public String getSaveDataFragment(String objectName) {
		...
		if (project.isBoundaryMode()) {
			...
			b.append("(" + SECURITY_MANAGER + ".getLogOnDTO()." + logOnDTO.getPKAttribute().getGetterName() + "));\n");
		}
		else {
			...
			b.append(".class, " + SECURITY_MANAGER + ".getLogOnDTO()." + logOnDTO.getPKAttribute().getGetterName() + "));\n");
		}
