...
/**
 * <p>
 * Edit part for domain attribute names
 * </p>
 * <p>
 ...
 * @version 1.0.0
 */
public class DomainAttributeNameEditPart extends CompartmentEditPart implements ITextAwareEditPart {
	 ...
	 * @param view
	 */
	public DomainAttributeNameEditPart(View view) {
		super(view);
	}
	...
	protected Image getLabelIcon() {
		...
		Image retImage = CodeCadenzaResourcePlugin.getImage(IMG_ATTRIBUTE);

		if (parserElement instanceof final DomainAttribute attribute) {
			if (attribute.isPk())
				retImage = CodeCadenzaResourcePlugin.getImage(IMG_ID_ATTRIBUTE);

			if (!attribute.isPersistent())
				return CodeCadenzaResourcePlugin.getImage(IMG_ATTRIBUTE);

			for (final DBIndex index : attribute.getColumn().getDatabaseTable().getIndexes())
				if (index.isUnique())
					for (final DBColumn c : index.getColumns())
						if (!attribute.isPk() && attribute.getColumn().equals(c)) {
							retImage = CodeCadenzaResourcePlugin.getImage(IMG_UK_ATTRIBUTE);
							break;
						}
		}
	...
	protected String getLabelText() {
		String text = null;
		final EObject parserElement = getParserElement();
		final var attribute = (DomainAttribute) parserElement;
	...
	public void setLabelText(String text) {
		...
		if (pdEditPolicy instanceof final CodeCadenzaTextSelectionEditPolicy textSelectionPolicy)
			textSelectionPolicy.refreshFeedback();
	}
	...
	private void performDirectEdit(char initialCharacter) {
		if (getManager() instanceof final TextDirectEditManager textManager)
			textManager.show(initialCharacter);
		else
			performDirectEdit();
	}
	...
	protected void setFontColor(Color color) {
		...
		else if (!attribute.isPersistent())
			getFigure().setForegroundColor(ColorConstants.darkGreen);
		else if (attribute.getDomainAttributeValidator().isNullable())
