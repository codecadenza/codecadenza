...
/**
 * <p>
 * Property section for many-to-one associations
 * </p>
 * <p>
 ...
 * @version 1.0.0
 */
public class M2OAssociationPropertySection extends AbstractPropertySection {
	...
	private ManyToOneAssociation association;
	private ManyToOneAssociationEditPart part;
		...
		public void widgetSelected(SelectionEvent e) {
			...
			part.getEditingDomain().getCommandStack().execute(new RecordingCommand(part.getEditingDomain()) {
				...
				protected void doExecute() {
					...
					association.getColumn().setNullable(association.isOptional());
				}
			});
	...
	public void createControls(Composite parent, TabbedPropertySheetPage aTabbedPropertySheetPage) {
		...
		data.left = new FormAttachment(0, LABEL_WIDTH);
		data.right = new FormAttachment(0, 400);
		data.top = new FormAttachment(chkCascadeRemove, ITabbedPropertyConstants.VSPACE);

		chkOptional = getWidgetFactory().createButton(panPropertySection, "", SWT.CHECK);
		chkOptional.setLayoutData(data);
		chkOptional.addSelectionListener(new SelectionChangeListener());

		data = new FormData();
		data.left = new FormAttachment(0, 0);
		data.right = new FormAttachment(chkOptional, -ITabbedPropertyConstants.HSPACE);
		data.top = new FormAttachment(chkOptional, 0, SWT.CENTER);

		final CLabel lblOptional = getWidgetFactory().createCLabel(panPropertySection, "Optional:");
		lblOptional.setLayoutData(data);

		data = new FormData();
		data.left = new FormAttachment(0, LABEL_WIDTH);
		data.right = new FormAttachment(0, 400);
		data.top = new FormAttachment(chkOptional, ITabbedPropertyConstants.VSPACE);

		chkInsertable = getWidgetFactory().createButton(panPropertySection, "", SWT.CHECK);
		chkInsertable.setLayoutData(data);
		chkInsertable.addSelectionListener(new SelectionChangeListener());
		...
		chkUpdatable.addSelectionListener(new SelectionChangeListener());
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		this.part = (ManyToOneAssociationEditPart) input;
		this.association = (ManyToOneAssociation) ((Edge) ((ManyToOneAssociationEditPart) input).getModel()).getElement();
	}
