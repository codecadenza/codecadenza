...
import net.codecadenza.eclipse.testing.dialog.project.EditRolesDialog;
...
class RoleTest {
	...
	void testRoleHandling() {
		...
		projectView.clickContextMenuEditRoles();
		new EditRolesDialog(bot, OperationMode.ADD).enterData();

		// Open the dialog for editing the role
		projectView.clickContextMenuEditRoles();
		new EditRolesDialog(bot, OperationMode.EDIT).enterData();

		// Open the dialog for removing the role
		projectView.clickContextMenuEditRoles();
		new EditRolesDialog(bot, OperationMode.REMOVE).enterData();

		// Delete the project in the 'Package Explorer' view
		new PackageExplorerView(bot, project).deleteProject();

		log.info("Finish role test");
	}
